...
const qbItems = questionBank.map(q => ({ 
    id: 'qb-' + q.id, 
...
window.renderCards = function() {
    ...
    var container = document.getElementById('qb-content');
    if (!container) container = document.getElementById('qbank-content');
    ...
    const pagContainer = document.getElementById('qb-pagination');
    if (pagContainer) {
...
window.updateLocalProgress = function() {
    var completed = qbItems.filter(q => STATE.completedQuestions.has(q.id)).length;
    var pct = qbItems.length > 0 ? Math.round((completed / qbItems.length) * 100) : 0;
    document.getElementById('progress-fill').style.width = pct + '%';
    document.getElementById('progress-text').innerText = completed + ' / ' + qbItems.length + ' (' + pct + '%)';
};

function updateFiltersUI() {
    let types = [...new Set(qbItems.map(i => i.type))];
    let topics = [...new Set(qbItems.map(i => i.topic || 'General'))].filter(Boolean);
